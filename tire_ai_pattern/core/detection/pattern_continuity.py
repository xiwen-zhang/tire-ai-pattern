...
import logging
from tire_ai_pattern.common.exceptions import InputDataError, RuntimeProcessError
...
# ============================================================
# 本地配置数据类（不依赖 configs 层）
# ============================================================

@dataclass
class PatternContinuityConfig:
    """图案连续性检测配置（算法层本地定义，脱离 configs 依赖）"""
...
def detect_pattern_continuity(
    ...
    Raises:
        InputDataError: 当输入图像或方法参数不符合要求时
        RuntimeProcessError: 当图案检测过程失败时
    """
    ...
    if len(image.shape) != 2:
        raise InputDataError("image", "shape", "expected 2D grayscale", image.shape)
    ...
    except (InputDataError, RuntimeProcessError):
        raise
    except Exception as e:
        raise RuntimeProcessError("detect_pattern_continuity", "边缘端点提取失败", e)
    ...
    try:
        ...
        raise RuntimeProcessError("_match_ends", "端点匹配失败", e)
    ...
    try:
        ...
        if image.shape[0] < 2 * config.edge_height:
            raise InputDataError(
                "image", "shape",
                f"expected height >= {2 * config.edge_height}",
                image.shape[:2]
            )
        ...
        raise RuntimeProcessError("_detect_with_method_a", "方法A检测失败", e)
...
def _detect_with_method_b(
    ...
    try:
        ...
        try:
            ...
            raise RuntimeProcessError("cv2.threshold", "二値化失败", e)
        ...
        raise RuntimeProcessError("_detect_with_method_b", "方法B检测失败", e)
