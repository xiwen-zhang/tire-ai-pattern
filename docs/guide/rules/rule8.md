...
`src.rules.executors.rule8` 实现了 Rule 8 的规则层逻辑，负责统计轮胎大图中的横沟数量，并输出相应评分。该模块属于规则层（rule layer），只负责规则判定与特征/得分的结构化表达，不直接实现底层图像算法。

## 适用场景

- 大图评估链路：对每张大图调用规则层，判断是否检测到横沟，并给出 Rule 8 得分。
